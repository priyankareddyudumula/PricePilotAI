...
def test_market_intelligence_engine_stats(app):
    ...
    with app.app_context():
        # Prices: 100, 110, 120 (Mean = 110, Median = 110, Min = 100, Max =
        # 120, StdDev = 8.16)
...
def test_competitive_positioning_and_risk_classification(app):
    ...
    with app.app_context():
        ...
        label1, _ = MarketIntelligenceEngine.classify_positioning(
            85.0, metrics, competitor_count=3)
        assert label1 == "Market Leader"

        # Aggressive Pricing: > 10% below median
        label2, _ = MarketIntelligenceEngine.classify_positioning(
            85.0, metrics, competitor_count=1)
        assert label2 == "Aggressive Pricing"

        # At Market: within 3% of median
        label3, _ = MarketIntelligenceEngine.classify_positioning(
            101.0, metrics, competitor_count=2)
        assert label3 == "At Market"

        # Premium: > 12% above median
        label4, _ = MarketIntelligenceEngine.classify_positioning(
            120.0, metrics, competitor_count=2)
...
def test_trend_detection_rolling_averages(app):
    ...
    with app.app_context():
        ...
        CompetitorService.ingest_price_records([{'competitor_name': 'Comp A',
                                                 'competitor_sku': 'SKU-A',
                                                 'internal_product_sku': 'TREND-TEST-SKU',
                                                 'title': 'P1',
                                                 'price': 140.0},
                                                {'competitor_name': 'Comp B',
                                                 'competitor_sku': 'SKU-B',
                                                 'internal_product_sku': 'TREND-TEST-SKU',
                                                 'title': 'P1',
                                                 'price': 160.0}])

        trend_res = TrendAndOpportunityEngine.calculate_rolling_trends(
            "TREND-TEST-SKU", days=30)
        assert trend_res['product_sku'] == "TREND-TEST-SKU"
        assert trend_res['rolling_7d_avg'] == 150.0
        assert trend_res['rolling_30d_avg'] == 150.0
        assert trend_res['trend_direction'] in [
...
def test_opportunity_detection_and_recommendations(app):
    ...
    with app.app_context():
        ...
        CompetitorService.ingest_price_records([{'competitor_name': 'Comp Low 1',
                                                 'competitor_sku': 'CL-1',
                                                 'internal_product_sku': 'OPP-LOW-SKU',
                                                 'title': 'Item',
                                                 'price': 100.0},
                                                {'competitor_name': 'Comp Low 2',
                                                 'competitor_sku': 'CL-2',
                                                 'internal_product_sku': 'OPP-LOW-SKU',
                                                 'title': 'Item',
                                                 'price': 100.0}])

        opp_rec = TrendAndOpportunityEngine.evaluate_product_opportunities(
            p_low)
        ...
        saved = PriceRecommendation.query.filter_by(
            product_id=p_low.id).first()
...
def test_market_intelligence_apis(client, app):
    ...
    with app.app_context():
        ...
        CompetitorService.ingest_price_records(
            [
                {
                    'competitor_name': 'API Comp',
                    'competitor_sku': 'AC-1',
                    'internal_product_sku': 'MARKET-API-SKU',
                    'title': 'Item',
                    'price': 100.0}])
