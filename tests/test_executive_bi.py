...
def test_executive_bi_kpis_and_drilldown(app):
    ...
    with app.app_context():
        p1 = Product(
            product_id="EXEC-SKU-1",
            current_price=150.0,
            cost_price=80.0)
        p2 = Product(
            product_id="EXEC-SKU-2",
            current_price=90.0,
            cost_price=70.0)
        ...
        drilldown = ExecutiveBIService.get_hierarchical_drilldown(
            dimension='category')
...
def test_executive_report_generation(app):
    ...
    with app.app_context():
        ...
        for rtype in report_types:
            # 1. PDF Generation
            pdf_bytes, pdf_mime, pdf_fname = ExecutiveReportService.generate_report(
                rtype, 'pdf')
            ...
            # 2. Excel Generation
            xlsx_bytes, xlsx_mime, xlsx_fname = ExecutiveReportService.generate_report(
                rtype, 'xlsx')
            ...
            # 3. CSV Generation
            csv_bytes, csv_mime, csv_fname = ExecutiveReportService.generate_report(
                rtype, 'csv')
...
def test_executive_bi_apis(client, app):
    ...
    res_rep_pdf = client.get(
        '/api/reports/export?report_type=Executive%20Summary&format=pdf')
    assert res_rep_pdf.status_code == 200
    assert res_rep_pdf.mimetype == 'application/pdf'

    # 4. GET /api/reports/export (CSV)
    res_rep_csv = client.get(
        '/api/reports/export?report_type=Revenue%20Report&format=csv')
